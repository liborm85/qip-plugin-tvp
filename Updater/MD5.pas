...
type
    ...
    TMD5Digest = array[0..15] of Byte;//Char; //x070410...
...
function StrMD5(const Buffer : AnsiString): AnsiString;
function MD5Chain(const args: array of AnsiString): AnsiString; // chain:args:separated:with:colons
{$endif !TASKUTIL}
...
function FmtHexNibble(pc: PAnsiChar; B: Byte): PAnsiChar;
begin
  case B of
    0..9: pc^:=AnsiChar(B+ord('0'));
    $A..$F: pc^:=AnsiChar(B+ord('a')-10);
    else pc^:='x';
  end;
  Result:=pc;
end;

procedure FmtHexByte(pc: PAnsiChar; B: Byte);
begin
  FmtHexNibble(FmtHexNibble(pc,B shr 4)+1,B and $0F);
end;

function Md5DigestToStr(const Digest: TMd5Digest): AnsiString;
var Buffer: array[0..63] of AnsiChar;
    pc: PAnsiChar;
    i: integer;
begin
  pc:=Buffer;
  for i:=0 to 15 do begin
    FmtHexByte(pc,Byte(Digest[i]));
    inc(pc,2);
  end;
  pc^:=#0;
  pc:=Buffer;
  SetString(Result,pc,32);
end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function StrMD5(const Buffer : AnsiString): AnsiString;
begin
    Result := GetMD5(Pointer(Buffer), Length(Buffer));
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function MD5Chain(const args: array of AnsiString): AnsiString;
var i,len,la,last: integer;
    cat: AnsiString;
    pc: PAnsiChar;
begin
  len:=0;
  for i:=low(args) to high(args) do
    inc(len,Length(args[i])+1); // include ':' for each arg...
  dec(len);
  SetString(cat,PAnsiChar(nil),len); //SetLength(cat,len);
  pc:=Pointer(cat);//pc:=@cat[1];
  last:=high(args);
  for i:=low(args) to high(args) do begin
    la:=Length(args[i]);
    Move(Pointer(args[i])^,pc^,la);
    inc(pc,la);
  ...
  //SetLength(cat,len-1); // eat last ':'
  Result:=LowerCase(StrMD5(cat));
end;
{$endif !TASKUTIL}
